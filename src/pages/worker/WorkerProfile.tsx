...
const WorkerProfile = () => {
  ...
  const [profileData, setProfileData] = useState({
    ...
    experience: "5",
    bio: "Tukang las berpengalaman dengan keahlian di bidang konstruksi baja dan aluminium.",
    availability: "available",
    photoUrl: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200",
  });

  const [selectedSkills, setSelectedSkills] = useState<string[]>(["Las/Welding", "Konstruksi Baja"]);
  ...
  const handleSkillToggle = (skill: string) => {
    if (selectedSkills.includes(skill)) {
      ...
      toast({ title: "Maksimal 5 Keahlian", description: "Anda hanya bisa memilih maksimal 5 keahlian", variant: "destructive" });
    }
  };

  const handleSave = () => {
    toast({ title: "Profil Tersimpan", description: "Perubahan profil Anda berhasil disimpan" });
  };

  return (
    <WorkerLayout title="Edit Profil">
        ...
        <div className="card-elevated p-6 md:p-8">
          ...
          <div className="flex items-center gap-6 mb-8 pb-8 border-b border-border">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Foto yang jelas akan meningkatkan kepercayaan pemberi kerja</p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Textarea id="bio" rows={4} value={profileData.bio} onChange={(e) => setProfileData({ ...profileData, bio: e.target.value })} placeholder="Ceritakan tentang pengalaman dan keahlian Anda..." />
            </div>

            <div className="space-y-4">
              <div>
                <Label>Keahlian (Pilih max. 5)</Label>
                <p className="text-sm text-muted-foreground">{selectedSkills.length}/5 keahlian dipilih</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {selectedSkills.map((skill) => (
                  <Badge key={skill} className="cursor-pointer" onClick={() => handleSkillToggle(skill)}>{skill} ✕</Badge>
                ))}
              </div>
              <div className="p-4 bg-muted/50 rounded-xl max-h-48 overflow-y-auto">
                <div className="flex flex-wrap gap-2">
                  {allSkills.filter((skill) => !selectedSkills.includes(skill)).map((skill) => (
                    <Badge key={skill} variant="outline" className="cursor-pointer hover:bg-primary hover:text-primary-foreground" onClick={() => handleSkillToggle(skill)}>+ {skill}</Badge>
                  ))}
                </div>
              </div>
            </div>

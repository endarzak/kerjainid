...
const menuItems = [
  { label: "Dashboard", icon: LayoutDashboard, path: "/dashboard/employer" },
  { label: "Pasang Lowongan", icon: PlusCircle, path: "/dashboard/employer/lowongan/baru" },
  { label: "Kelola Lowongan", icon: Briefcase, path: "/dashboard/employer/lowongan" },
  { label: "Cari Pekerja", icon: Search, path: "/cari-pekerja" },
  { label: "Pengaturan", icon: Settings, path: "/dashboard/employer/pengaturan" },
];

const EmployerLayout = ({ children, title }: EmployerLayoutProps) => {
    ...
    <>
      <div className="p-4 border-b border-border">
        <Link to="/dashboard/employer" className="flex items-center gap-2">
          ...
          {!collapsed && (
            <span className="text-xs bg-accent/10 text-accent px-2 py-0.5 rounded">Employer</span>
          )}
        </Link>
      </div>
      ...
      <div className="p-4 flex-1 overflow-y-auto">
        {!collapsed && (
          <div className="mb-4 px-3">
            <p className="font-medium text-foreground text-sm truncate">{user?.companyName || user?.fullName}</p>
        ...
        {!collapsed && (
          <div className="mt-6 p-3 bg-accent/5 border border-accent/20 rounded-lg">
            <Crown className="h-5 w-5 text-accent mb-2" />
            <p className="text-xs font-medium text-foreground mb-1">Upgrade Premium</p>
            <p className="text-xs text-muted-foreground mb-2">Akses fitur unlimited</p>
